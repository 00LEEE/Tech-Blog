...
router.get('/', async (req, res) => {
  ...
  res.render('pages/home', { title: 'Home', posts, user: req.session?.user?.username });
});
...
router.delete('/posts/:id', async (req, res) => {
  ...
  if (req.session?.user && post && post.userId === req.session.user.id) {
    await post.destroy();
  }
  res.redirect(303, '/');
});
...
router.delete('/comments/:id', async (req, res) => {
  ...
  if (req.session?.user && comment && comment.userId === req.session.user.id) {
